# <a id="HDBWellFormedness"></a>[HDB Well-Formedness](#HDBWellFormedness)
...
### <a id="Version"></a>[Version](#Version)

This work documents the behavior of q 3.6 2019.06.05.

### <a id="Heuristic"></a>[Heuristic](#Heuristic)
...
### <a id="Approach"></a>[Approach](#Approach)
...
An HDB is a directory containing only any<sup>[1](#noteEmptyHDB)</sup> of the following elements:

 * Files or directories whose names end in a dollar sign (`x like"*$"`) ([staging files](#StagingFiles))

 * [Code](#Code)

 * [Serialized q data](#SerializedqData)

 * [Splayed tables](#SplayedTables)

 * [Partitions](#Partitions) ***or*** the file `par.txt` pointing to [segments](#Segments)

 * A directory named [`html`](#HTML)

 * [Other directories](#OnNesting)
...
### <a id="StagingFiles"></a>[Staging Files](#StagingFiles)
...
As q is currently shipped, it is an error for such a file to have an extension other than `.q` or `.k`, as these are the only languages recognized.<sup>[2](#noteBadCode)</sup>

### <a id="SerializedqData"></a>[Serialized q Data](#SerializedqData)
...
### <a id="SplayedTables"></a>[Splayed Tables](#SplayedTables)
...
### <a id="Partitions"></a>[Partitions](#Partitions)
...
The partitioning field of a partitioned HDB is determined by the name of the first partition: if it is 10, 7, or 4 characters long, the field is `` `date``, `` `month``, or `` `year``, respectively; otherwise it is `` `int``.<sup>[3](#noteIntLong), [4](#noteIntYear)</sup>
...
It is an error for all partitions not to be of the same [schema](#OnSchemas): for each table present in any partition, it must exist with the same schema in all partitions.<sup>[5](#note.Q.bv)</sup>

Note that it is ***not*** an error for all partitions to be empty.

### <a id="Segments"></a>[Segments](#Segments)
...
The partitioning field of a segmented HDB is the partitioning field of its first non-empty segment, where segments are ordered by their order in `par.txt`.<sup>[6](#noteIntYearSeg)</sup>
...
### <a id="OnNesting"></a>[On Nesting](#OnNesting)
...
## <a id="Footnotes"></a>[Footnotes](#Footnotes)
...
4. <a id="noteIntYear"></a>An HDB meant to be partitioned by `int` is loaded as `year`-partitioned if its first partition's name is a four-digit number. A dummy partition named `0` or `1` (containing [schema-conformant](#OnSchemas), but empty, tables) prevents this.
...
6. <a id="noteIntYearSeg"></a>The considerations in <sup>[4](#noteIntYear)</sup> also apply here.
